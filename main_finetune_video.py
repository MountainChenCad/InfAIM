# 文件: main_finetune_video.py (版本 4 - 降低学习率)
# 核心改动:
# 1. (BUG FIX) 将默认学习率从 1e-4 降低到 1e-5。这是解决损失不收敛问题的最关键步骤。
...
def main(args):
    ...
    dataset_val = VideoDetectionDataset(data_root=args.data_path, transform=transform, mode='val', return_clips=False)
    sampler_val = torch.utils.data.SequentialSampler(dataset_val)
    data_loader_val = DataLoader(dataset_val, sampler=sampler_val, batch_size=1, num_workers=args.num_workers,
                                 collate_fn=collate_fn_val)

    dataset_train_vis = VideoDetectionDataset(data_root=args.data_path, transform=transform, mode='train',
                                              return_clips=False)
    sampler_train_vis = torch.utils.data.SequentialSampler(dataset_train_vis)
    data_loader_train_vis = DataLoader(dataset_train_vis, sampler=sampler_train_vis, batch_size=1,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and misc.is_main_process():
            ...
            visualize_epoch(model_without_ddp, data_loader_val, device, args.output_dir, epoch, 'val_visuals')
